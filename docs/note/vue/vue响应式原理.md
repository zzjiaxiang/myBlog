...
至此,我们观察实现 **03-mini-observer** 和 **05-activeEffect(优化)**,发现响应式的实现很类似.数据的劫持一个是用的 Proxy,一个是用 Object.defineProperty.另外 vue2 里面用 Dep 类将 notify 和 depend 函数以及 subscribers 封装起来了, 而 vue3 里面单独将 notify(trigger) 和 depend(tarck) 拆开来.

实际上 notify 和 trigger, depend 和 tarck 他们仍在做同样的事情.我们知道在 vue2 里面是一个 Dep 类来封装的,可以说一个依赖实例被依赖(depend)或者说正在通知(notify)它的订阅者,那么在 vue3 里面已经没有了 Dep 类,可以说正在 track(跟踪)什么东西,而不是什么东西正在被依赖(depend).
